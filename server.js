...
const mitm = async req => {
    ...
    if (!url.startsWith('http://')) return req.respond({ status: 400, body: 'invalid' })
    headers.delete('proxy-connection')

    try {
        const res = await fetch(url, { method, headers, body: await readBody(req) })
...
const tunnel = async req => {
    ...
    await req.w.write(encoder.encode(`${req.proto} 200 Connection established\r\n\r\n`))
    await req.w.flush()

    let conn = null
    try {
        conn = await Deno.connect({ hostname, port: parseInt(port || 443), transport: 'tcp' })
    } catch (e) {
        await req.finalize()
    }
    if (conn) {
...
console.log(`HTTP proxy listening on http://${options.hostname}:${options.port}/`)
